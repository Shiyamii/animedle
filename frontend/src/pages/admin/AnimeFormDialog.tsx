...
export function AnimeFormDialog({ vm }: { vm: ReturnType<typeof useAdminViewModel> }) {
    ...
    const { form, error, editingId, isLoading, updateFormField, updateTitle, addTitle, removeTitle, handleSubmit, closeForm } = vm;

    return (
        <Dialog open={vm.showForm} onOpenChange={(open) => { if (!open) closeForm(); }}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        />
                        <p className="text-xs text-muted-foreground">{t('admin.form.imageUrlHint')}</p>
                    </div>

                    <div className="flex flex-col gap-2">
                        <div className="flex items-center justify-between">
                            <label className="text-sm font-medium">{t('admin.form.titles')}</label>
                            <button
                                type="button"
                                onClick={addTitle}
                                className="text-xs text-primary hover:underline flex items-center gap-1"
                            >
                                <Plus className="w-3 h-3" /> {t('admin.form.addTitle')}
                            </button>
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="flex flex-col gap-1.5">
                            <label className="text-sm font-medium">{t('admin.form.format')}</label>
                            <select
                        ...
                        <div className="flex flex-col gap-1.5">
                            <label className="text-sm font-medium">{t('admin.form.demographic')}</label>
                            <select
                            ...
                            >
                                <option value="">—</option>
                                {DEMOGRAPHIC_OPTIONS.map(d => <option key={d} value={d}>{d}</option>)}
                            </select>
                        </div>

                        <div className="flex flex-col gap-1.5">
                            <label className="text-sm font-medium">{t('admin.form.episodes')}</label>
                            <Input
                                type="number"
                                placeholder="12"
                                value={form.episodes}
                        ...
                        <div className="flex flex-col gap-1.5">
                            <label className="text-sm font-medium">{t('admin.form.score')}</label>
                            <Input
                                type="number"
                                step="0.01"
                                placeholder="7.80"
                        ...
                        <div className="flex flex-col gap-1.5">
                            <label className="text-sm font-medium">{t('admin.form.seasonStart')}</label>
                            <Input
                        ...
                        <div className="flex flex-col gap-1.5">
                            <label className="text-sm font-medium">{t('admin.form.studio')}</label>
                            <Input
                        ...
                        <div className="flex flex-col gap-1.5">
                            <label className="text-sm font-medium">{t('admin.form.source')}</label>
                            <select
                                value={form.source}
                    ...
                    <div className="flex flex-col gap-1.5">
                        <label className="text-sm font-medium">{t('admin.form.genres')}</label>
                        <Input
                            placeholder={t('admin.form.genresPlaceholder')}
                            value={form.genres}
                            onChange={e => updateFormField('genres', e.target.value)}
                        />
                        <p className="text-xs text-muted-foreground">{t('admin.form.genresHint')}</p>
                    </div>
                </div>
