...
export function useAdminViewModel() {
  ...
  const filteredAnimes = useMemo(() => {
    ...
    return animes.filter((anime) => {
      if (enabledFilter === 'enabled' && !anime.enabled) {
        return false;
      }
      if (enabledFilter === 'disabled' && anime.enabled) {
        return false;
      }
      if (q && !anime.title.toLowerCase().includes(q)) {
        return false;
      }
      return true;
    });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        episodes: parseInt(form.episodes, 10) || 0,
      ...
      const url = editingId ? `${API_URL}/api/admin/animes/${editingId}` : `${API_URL}/api/admin/animes`;
