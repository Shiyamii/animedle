...
const { mockNavigate, mockSetUser, mockClearUser, mockUpdateUser, mockChangePassword, mockSignOut, mockUseSession } =
  vi.hoisted(() => ({
    mockNavigate: vi.fn(),
    mockSetUser: vi.fn(),
    mockClearUser: vi.fn(),
...
describe('useAccountViewModel', () => {
  ...
  describe('updateProfile', () => {
    it('appelle authClient.updateUser avec name et avatarSeed', async () => {
      ...
      expect(mockUpdateUser).toHaveBeenCalledWith(expect.objectContaining({ name: 'Bob' }));
    });
    ...
    it('met à jour le store utilisateur en cas de succès', async () => {
      ...
      expect(mockSetUser).toHaveBeenCalledWith(expect.objectContaining({ id: 'user-1', email: 'alice@example.com' }));
    });
  ...
  describe('randomizeAvatar', () => {
    it('génère un nouveau seed aléatoire', () => {
      const { result } = renderHook(() => useAccountViewModel());
      act(() => {
        result.current.randomizeAvatar();
      });
