...
describe('AnimeService', () => {
    ...
    describe('parseMALImageUrl', () => {
        ...
        it('lève une erreur pour une URL non-MAL', () => {
            const service = getService() as unknown as WithParse;
            expect(() => service.parseMALImageUrl('https://invalid.com/image.jpg'))
                .toThrow('URL MAL invalide');
        });
    ...
    describe('guessAnime', () => {
        ...
        it('retourne isCorrect=false avec les bons résultats de comparaison', async () => {
            const current = makeEntity({
                _id: new Types.ObjectId(),
                episodes: 24, score: 8.5, season_start: 'Spring 2023',
                genres: ['Action', 'Adventure'], studio: 'MAPPA',
                source: 'Manga', anime_format: 'TV', demographic_type: 'Shounen',
            });
            const guessed = makeEntity({
                _id: new Types.ObjectId(),
                episodes: 12, score: 7.0, season_start: 'Fall 2020',
                genres: ['Action'], studio: 'WIT Studio',
                source: 'Original', anime_format: 'Movie', demographic_type: 'Seinen',
            });
            setupCurrentAnime(current);
            mocks.findById.mockResolvedValue(guessed);

            const { isCorrect, guessNumber, results } = await getService().guessAnime(
                guessed._id!.toHexString(), 3
            );
    ...
    describe('comparaison épisodes et score', () => {
        async function getResults(
            refEps: number, testEps: number,
            refScore: number, testScore: number
        ) {
