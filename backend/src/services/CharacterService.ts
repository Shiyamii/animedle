...
export class CharacterService {
  ...
  private getCharacterMainAnimeTitle(entity: CharacterEntity): string {
    return entity.anime_titles.find((title) => title.type === 'Default')?.title ?? entity.anime_titles[0]?.title ?? '';
  }
  ...
  private getCurrentCharacter(): Promise<CharacterEntity> {
    return this.currentCharacterRepository
      .getCurrentCharacter()
      .then(async (current) => {
