...
export class AnimeService {
  ...
  private parseMALImageUrl(url: string): AnimeImagesWebpEntity {
    ...
    if (!match) {
      throw new Error('URL MAL invalide. Format attendu : https://cdn.myanimelist.net/images/anime/{dossier}/{id}');
    }
    const base = match[1];
    return {
      image_url: `${base}.webp`,
  ...
  async createAnime(data: CreateAdminAnimeDTO): Promise<AdminAnimeDTO> {
    const imagesWebp = this.parseMALImageUrl(data.imageUrl);
    const entity = await this.repository.create({
      images_webp: imagesWebp,
      titles: data.titles,
  ...
  private getMainTitle(entity: AnimeEntity): string {
    return entity.titles.find((t) => t.type === 'Default')?.title ?? entity.titles[0]?.title ?? '';
  }

  private getAliases(entity: AnimeEntity): string[] {
    return entity.titles.filter((t) => t.type !== 'Default').map((t) => t.title);
  }
   ...
   */
  private getCurrentAnime(): Promise<AnimeEntity> {
    return this.currentAnimeRepository
      .getCurrentAnime()
      .then(async (current) => {
  ...
  public getAnimeStats(animeIds: string[]): Promise<AnimeStatsDTO[]> {
  ...
  public async getAdminStats(): Promise<AdminStatsDTO> {
    ...
    const todayTotalGuesses = today ? Object.values(today.guesses ?? {}).reduce((a, b) => a + b, 0) : 0;
    ...
    return {
      ...
      results: {
        ...
        episodes: {
          ...
          isHigher: test.episodes < ref.episodes,
        },
        seasonStart: {
          isCorrect: ref.season_start === test.season_start,
          isEarlier: this.compareDates(test.season_start, ref.season_start) > 0,
        },
        studio: {
